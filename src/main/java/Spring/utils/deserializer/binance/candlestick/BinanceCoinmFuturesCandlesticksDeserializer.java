...
public class BinanceCoinmFuturesCandlesticksDeserializer implements JsonDeserializer<BinanceCandlesticks> {
    @Override
    public BinanceCandlesticks deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        data.setBaseAssetVolume(jsonArray.get(7).getAsString());
        data.setNumberOfTrades(jsonArray.get(8).getAsString());
        data.setTakerBuyQuoteAssetVolume(jsonArray.get(9).getAsString());
        data.setTakerBuyBaseAssetVolume(jsonArray.get(10).getAsString());
